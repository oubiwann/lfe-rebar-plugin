PROJECT = lfe-rebar-plugin
LIB = lfe-rebar-plugin
DEPS = ./deps
BIN_DIR = ./bin
EXPM = $(BIN_DIR)/expm
LFE_DIR = $(DEPS)/lfe
...
run:
	@ERL_LIBS=$(ERL_LIBS) $(LFE) -eval "application:start('lfe-rebar-plugin')"

daemon:
	@ERL_LIBS=$(ERL_LIBS) $(LFE) \
	-eval "application:start('lfe-rebar-plugin')" -detached -noshell
